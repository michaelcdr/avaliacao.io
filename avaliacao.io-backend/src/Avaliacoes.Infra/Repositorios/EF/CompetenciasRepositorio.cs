...
namespace Avaliacoes.Infra.Repositorios.EF
{
    public class CompetenciasRepositorio : Repositorio<Competencia>, ICompetenciasRepositorio
    {
        ...
        public async Task<bool> Existe(int disciplinaId, string nome, int? idCompetencia)
        {
            return idCompetencia == null
                ? await Context.Set<Competencia>().AnyAsync(e => e.DisciplinaId == disciplinaId && e.Nome == nome)
                : await Context.Set<Competencia>().AnyAsync(e => e.DisciplinaId == disciplinaId && e.Nome == nome && e.Id != idCompetencia);
        } 
    }
}
